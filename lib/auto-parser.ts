...
export class AutoParser {
  ...
  async parseFile(filename: string, content: string): Promise<AutoParseResult> {
    try {
      // ANTLR4 parser temporarily disabled - generated files not available
      // TODO: Re-enable when UnifiedSpiralParser is generated
      ...
      console.warn('ANTLR4 parser disabled, using legacy parser:', error instanceof Error ? error.message : String(error));
    }
