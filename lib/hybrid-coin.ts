...
export interface HybridCoinTransaction {
  ...
  metadata?: {
    ...
    rewardSource?: string;
    // IBC-related metadata
    sourceChannel?: string;
    timeout?: number;
    ibcDenom?: string;
    // Governance metadata
    proposalId?: string;
    vote?: 'yes' | 'no' | 'abstain' | 'veto' | 'noWithVeto';
    votingPower?: number;
  };
}
...
export class HybridCoin {
  ...
  private metrics!: HybridCoinMetrics; // Initialized in constructor
  private transactions: Map<string, HybridCoinTransaction> = new Map();
  private stakingPositions: Map<string, StakingPosition> = new Map();
  private distribution!: HybridCoinDistribution; // Initialized in constructor
